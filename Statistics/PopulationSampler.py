...
class PopulationSampler(RandomGenerator):
    ...
    def get_margin_of_error(self, data):
        margin_of_error = self.stats.divide(
                                self.stats.multiply(
                                        self.stats.z_score(data)
                                            , (self.stats.standard_deviation(data))
                                )
    ...
    def get_result_by_cochrans_sample_size(self, p1, p_diff, alpha):
        ...
        n = 1
        while True:
            z = self.stats.z_score(p1, p1 + p_diff, n1=n, n2=n)
    ...
    def get_sample_size_by_confidence_interval_and_width(self, data):
        # step 1
        za_2 = self.stats.z_score(self.stats.divide(self.get_confidence_interval(self, data),2))
